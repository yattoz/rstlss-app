...
import com.rstlss.app.alarm.RadioSleeper
...
class RadioService : MediaBrowserServiceCompat() {
    ...
    private val receiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            ...
            if (action != null && action == AudioManager.ACTION_AUDIO_BECOMING_NOISY) {
                ...
                i.putExtra("action", com.rstlss.app.Actions.STOP.name)
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        when (intent.getStringExtra("action")) {
            com.rstlss.app.Actions.PLAY.name -> beginPlaying()
            com.rstlss.app.Actions.STOP.name -> { setVolume(PlayerStore.instance.volume.value); stopPlaying() } // setVolume is here to reset the volume to the user's preference when the alarm (that sets volume to 100) is dismissed
            com.rstlss.app.Actions.PAUSE.name -> { setVolume(PlayerStore.instance.volume.value); pausePlaying() }
            com.rstlss.app.Actions.VOLUME.name -> setVolume(intent.getIntExtra("value", 100))
            com.rstlss.app.Actions.KILL.name -> {stopForeground(true); stopSelf(); return Service.START_NOT_STICKY}
            com.rstlss.app.Actions.NOTIFY.name -> nowPlayingNotification.update(this)
            com.rstlss.app.Actions.PLAY_OR_FALLBACK.name -> beginPlayingOrFallback()
            com.rstlss.app.Actions.FADE_OUT.name -> {
            ...
            com.rstlss.app.Actions.CANCEL_FADE_OUT.name -> { handler.removeCallbacks(lowerVolumeRunnable) }
            com.rstlss.app.Actions.SNOOZE.name -> { RadioAlarm.instance.snooze(this) }
        }
    ...
    private fun beginPlayingOrFallback()
    {
        ...
        com.rstlss.app.Async(wait, post)
    }
